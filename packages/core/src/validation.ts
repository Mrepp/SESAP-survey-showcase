...
// Third-party providers embed an absolute URL; `r2` media is served from our
// own origin, so it is a root-relative path instead.
const EmbedUrlSchema = z.union([z.string().url(), z.string().regex(/^\/[^\s]*$/)]);

export const InterviewVideoSchema = z.object({
  provider: z.enum(['kaltura', 'youtube', 'vimeo', 'iframe', 'r2']),
...
/**
 * One atomic edit to an interview. Partial by design: a caller sends the slices
 * it changed, and anything absent is left alone.
 */
export const InterviewDraftSchema = z
  ...
  .partial()
  .refine((draft) => Object.values(draft).some((slice) => slice !== undefined), {
    message: 'Draft must contain at least one of: title, demographics, metadata, analysis',
  });
