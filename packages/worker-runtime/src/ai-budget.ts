import { AI_BUDGET_POOLS, KV_KEYS } from '@sesap/types';
import type { AiBudgetPool } from '@sesap/types';
import { SesapError } from '@sesap/core';

export type { AiBudgetPool };

/**
 * Durable Object instance per budget pool. The pipeline pool keeps the
 * historical instance name so an existing deployment's state carries over.
 */
export const AI_NEURON_LIMITER_DO_NAMES: Record<AiBudgetPool, string> = {
  pipeline: 'global',
  showcase: 'showcase',
};

/** The longest a queue message may be delayed; the platform caps `delaySeconds` at 12 hours. */
export const MAX_QUEUE_DELAY_SECONDS = 12 * 60 * 60;

/**
 * Seconds until the budget resets (00:00 UTC), plus a minute of slack so a
 * deferred run does not land a moment before the roll-over. Capped at the
 * platform's maximum delay.
 */
export function secondsUntilBudgetReset(now = new Date()): number {
  const nextMidnight = Date.UTC(
    now.getUTCFullYear(),
    now.getUTCMonth(),
    now.getUTCDate() + 1,
  );
  const seconds = Math.ceil((nextMidnight - now.getTime()) / 1000) + 60;
  return Math.max(60, Math.min(MAX_QUEUE_DELAY_SECONDS, seconds));
}
...
export interface AiBudgetRequest {
  ...
  /** Which daily budget to draw from. Defaults to the pipeline's. */
  pool?: AiBudgetPool;
  context?: JsonRecord;
}
...
export async function runWithAiBudget<T>(
  ...
  const pool = request.pool ?? 'pipeline';
  const reservation = await reserveAiNeurons(env, pool, {
    model: request.model,
  ...
  try {
    ...
    await settleAiNeuronReservation(env, pool, 'consume', reservation.reservationId);
    return result;
  } catch (err) {
    try {
      await settleAiNeuronReservation(env, pool, 'release', reservation.reservationId);
    } catch {
...
export class AiNeuronLimiter {
  ...
  async fetch(request: Request): Promise<Response> {
    ...
    try {
      ...
      if (request.method === 'GET' && url.pathname === '/status') {
        const pool = readPool(url.searchParams.get('pool'));
        const state = await this.getCurrentState(pool, await this.readMaxNeurons(pool));
  ...
  private async reserve(body: JsonRecord): Promise<Response> {
    ...
    const pool = readPool(body.pool);
    const context = isJsonRecord(body.context) ? body.context : undefined;
    const maxNeurons = await this.readMaxNeurons(pool);
    const now = new Date();
    const date = utcDate(now);

    // The KV snapshot is written after the transaction commits: a KV put inside
    // the closure would survive a rollback and misreport usage.
    let snapshot: LimiterState | undefined;
    const response = await this.state.storage.transaction(async (txn) => {
      const current = await getState(txn, pool, date, maxNeurons, now);
    ...
    if (snapshot) await this.writeSnapshot(snapshot);
    return response;
  }
  ...
  private async settle(body: JsonRecord, action: 'consume' | 'release'): Promise<Response> {
    ...
    const response = await this.state.storage.transaction(async (txn) => {
      ...
      if (reservation.date !== current.date) {
        // Reserved yesterday, settled today: the day's state has already
        // reset, so there is nothing to credit or debit — but the row must
        // not be left behind forever.
  ...
  private async readMaxNeurons(pool: AiBudgetPool): Promise<number> {
    const key = KV_KEYS.aiNeuronsDailyMax(pool);
  ...
  private async getCurrentState(pool: AiBudgetPool, maxNeurons: number): Promise<LimiterState> {
    return getState(this.state.storage, pool, utcDate(new Date()), maxNeurons, new Date());
  }

  private async writeSnapshot(state: LimiterState): Promise<void> {
    await this.env.SESAP_KV.put(
      KV_KEYS.aiNeuronsDailyUsage(state.date, state.pool),
...
async function reserveAiNeurons(
  env: AiBudgetEnv,
  pool: AiBudgetPool,
  body: { model: string; neurons: number; pool: AiBudgetPool; context?: JsonRecord },
): Promise<{ reservationId: string }> {
  const res = await limiterFetch(env, pool, '/reserve', body);
...
async function settleAiNeuronReservation(
  ...
  const res = await limiterFetch(env, pool, `/${action}`, { reservationId, pool });
  if (!res.ok) {
    const payload = await res.json() as ReserveResponse;
    throw budgetErrorFromResponse(payload, res.status);
  }
...
function limiterFetch(
  env: AiBudgetEnv,
  pool: AiBudgetPool,
  path: string,
  body: JsonRecord,
): Promise<Response> {
  const id = env.AI_NEURON_LIMITER.idFromName(AI_NEURON_LIMITER_DO_NAMES[pool]);
...
async function getState(
  ...
  if (current.maxNeurons !== maxNeurons || current.pool !== pool) {
    current.maxNeurons = maxNeurons;
    // States written before pools existed carry no `pool`; they are the pipeline's.
    current.pool = pool;
    current.updatedAt = now.toISOString();
...
function readPool(value: unknown): AiBudgetPool {
  if (value === undefined || value === null || value === '') return 'pipeline';
  if ((AI_BUDGET_POOLS as readonly string[]).includes(String(value))) return value as AiBudgetPool;
  throw new AiBudgetConfigError(`Unknown AI budget pool ${JSON.stringify(value)}`, {
    pool: value,
    allowed: AI_BUDGET_POOLS,
  });
}
