...
class MockKV {
  ...
  constructor(maxNeurons?: number, showcaseMaxNeurons?: number) {
    if (maxNeurons !== undefined) {
      this.store.set(KV_KEYS.aiNeuronsDailyMax(), String(maxNeurons));
    }
    if (showcaseMaxNeurons !== undefined) {
      this.store.set(KV_KEYS.aiNeuronsDailyMax('showcase'), String(showcaseMaxNeurons));
    }
  }
...
describe('AiNeuronLimiter', () => {
  ...
  it('reads the showcase pool its own cap and writes its own snapshot', async () => {
    const storage = new MockStorage();
    const kv = new MockKV(1000, 10);
    const limiter = new AiNeuronLimiter({ storage }, { SESAP_KV: kv });

    const first = await post(limiter, '/reserve', { model: 'test', neurons: 8, pool: 'showcase' });
    expect(first.status).toBe(200);
    const second = await post(limiter, '/reserve', { model: 'test', neurons: 8, pool: 'showcase' });
    expect(second.status).toBe(429);

    const date = new Date().toISOString().slice(0, 10);
    expect(kv.store.has(KV_KEYS.aiNeuronsDailyUsage(date, 'showcase'))).toBe(true);
    expect(kv.store.has(KV_KEYS.aiNeuronsDailyUsage(date))).toBe(false);
  });

  it('rejects an unknown pool', async () => {
    const { limiter } = createLimiter(100);
    const reserve = await post(limiter, '/reserve', { model: 'test', neurons: 1, pool: 'nope' });
    expect(reserve.status).toBe(503);
  });

  it('deletes a reservation settled after the day rolled over', async () => {
    const { limiter, storage } = createLimiter(100);
    const reserve = await post(limiter, '/reserve', { model: 'test', neurons: 10 });
    const { reservationId } = await reserve.json() as { reservationId: string };

    // Age the reservation and the state by a day.
    const reservation = storage.store.get(`reservation:${reservationId}`) as { date: string };
    storage.store.set(`reservation:${reservationId}`, { ...reservation, date: '2000-01-01' });

    const consume = await post(limiter, '/consume', { reservationId });
    expect(await consume.json()).toEqual({ ok: true, ignored: true });
    expect(storage.store.has(`reservation:${reservationId}`)).toBe(false);
  });
});

describe('secondsUntilBudgetReset', () => {
  it('counts to the next UTC midnight with a minute of slack', () => {
    expect(secondsUntilBudgetReset(new Date('2026-03-01T23:00:00Z'))).toBe(3600 + 60);
  });

  it('never exceeds the platform delay cap', () => {
    expect(secondsUntilBudgetReset(new Date('2026-03-01T00:00:30Z'))).toBe(MAX_QUEUE_DELAY_SECONDS);
  });
});

describe('runWithAiBudget', () => {
  it('addresses the pool it was asked for', async () => {
    const names: string[] = [];
    const limiter = {
      ...
      get: () => ({
        fetch: async () => new Response(JSON.stringify({ ok: true, reservationId: 'res-1' }), {
          headers: { 'Content-Type': 'application/json' },
        }),
      }),
    };
    ...
    expect(names).toEqual([
      AI_NEURON_LIMITER_DO_NAMES.showcase,
      AI_NEURON_LIMITER_DO_NAMES.showcase,
    ]);
  });
